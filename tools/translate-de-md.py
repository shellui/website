#!/usr/bin/env python3
"""Translate markdown locale files (prose only, preserve code/links)."""
...
def restore(text: str, slots: list[str]) -> str:
    for i, val in enumerate(slots):
        text = text.replace(f"ZZZSLOT{i:04d}ZZZ", val)
    return text
...
def tr(line: str) -> str:
    ...
    if not re.search(r"[A-Za-z]{4}", p):
        return line
    try:
        out = argostranslate.translate.translate(p, "en", "de")
...
def main() -> None:
    for p in [
        ROOT / "content/locales/de/guidelines/design.md",
        ROOT / "content/locales/de/guidelines/web-design.md",
        ROOT / "content/locales/de/guidelines/writing.md",
        ROOT / "src/de/blog/introducing-shellui.md",
        ROOT / "src/de/company/contribute/index.md",
        ROOT / "src/de/company/legal/index.md",
    ]:
        translate_md(p)
