...
| `changelog/index.njk` | `item` | Build-time markdown → limited inline HTML via `src/_data/changelog.js` |
...
`src/_data/changelog.js` converts upstream release notes to a **small allowlist** of inline tags (`<a>`, `<code>`, `<strong>`, `<em>`). Plain text is HTML-escaped before inline markdown runs. Curated entries in `CURATED` follow the same path.
